...
function generateClassSchedules(selectedClasses, classData) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      document.getElementById('output').textContent = 'You cannot currently select "Free Any" along wing another free block';
      resolve([]); // Resolve with an empty array to indicate no valid schedules found
    ...
    clearTimeout(timeout); // Clear the timeout if schedules are found before 5 seconds
    resolve(schedules);
  });
}
